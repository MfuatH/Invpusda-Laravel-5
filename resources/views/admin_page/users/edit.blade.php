...
<head>
    ...
    <title>Edit Existing User // DATA CORRUPTION</title>
    <!-- Load Tailwind CSS -->
    <script src="https://cdn.tailwindcss.com"></script>
    <style>
        /* Unsettling Patterned Background (Lime and Dark Purple) */
        body {
    ...
    <div class="max-w-4xl mx-auto mt-12 mb-20">
        ...
        ">
            ...
            <h1 class="text-3xl md:text-5xl font-extrabold text-lime-400 tracking-widest uppercase mb-2">
                EDIT<span class="text-pink-400 text-6xl">::</span>ENTITY
            </h1>
            <p class="text-pink-200 text-lg mb-8 font-light italic border-b border-pink-400 pb-4">
                Modify the entity's parameters. Identity is fragile. Current ID: {{ $user->id }}
            </p>
            ...
            <form action="{{ route('super.users.update', $user) }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="col-span-1">
                    <label for="password" class="block text-sm font-medium uppercase text-pink-400 tracking-widest">
                        Access KEY (Min. 8 bits) <span class="text-xs italic text-gray-400">(Leave blank to maintain old key)</span>
                    </label>
                ...
                <div class="col-span-1 mt-6 -rotate-1">
                    ...
                    <select name="role" id="role" required 
                        ...
                        <option value="" disabled class="text-gray-500">SELECT REALITY LEVEL</option>
                        @foreach ($roles as $role)
                            <option value="{{ $role }}" {{ old('role', $user->role) == $role ? 'selected' : '' }} class="bg-purple-900 text-lime-300">
                                {{ $role }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-span-1 mb-8">
                    ...
                    <select name="bidang_id" id="bidang_id" required 
                        ...
                        <option value="" disabled class="text-gray-500">SELECT NULL SPACE</option>
                        <!-- Simulating Blade loop for bidangs -->
                        @foreach ($bidangs as $bidang)
                            <option value="{{ $bidang->id }}" {{ old('bidang_id', $user->bidang_id) == $bidang->id ? 'selected' : '' }} class="bg-purple-900 text-lime-300">
